...
if __name__ == "__main__":
    ...
    annual_rate = 0.054          # 5.4%
    amort_months = 240           # 20 years
    maturity_months = 60         # 5 years (balloon at 60 months)
    ...
    sched = amortization_schedule(
        principal, annual_rate, amort_years, maturity_years,
        payments_per_year=payments_per_year, round_to_cents=True
    )

    df = pd.DataFrame([asdict(r) for r in sched])
    df.index = df['period']
    df_display = df[['payment','interest','principal','balance_end','balloon_due']].copy()
