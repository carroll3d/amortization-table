...
@dataclass
class Curve:
    periods: np.ndarray   # strictly increasing x (e.g., months, years)
    rates: np.ndarray     # y in decimals (e.g., 0.054 == 5.4%)
    interpolator: PchipInterpolator

def make_curve(known_periods: Iterable[Number], known_rates: Iterable[Number]) -> Curve:
    """
    Build a shape-preserving PCHIP curve from known (period, rate) points.
    known_rates can be in % or decimals; auto-converted to decimals if needed.
    """
    ...
    # If values look like percents, convert to decimals
    if np.nanmax(y) > 1.0:
        y = y / 100.0

    # Deduplicate by averaging rates for identical periods
